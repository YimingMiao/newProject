...
public class GraphicsGroup extends GraphicsObject implements GraphicsObserver {
    ...
    public boolean testHit(double x, double y, Graphics2D gc){
        Rectangle test = new Rectangle((int)Math.round(x), (int)Math.round(y), 1,1);
        for(GraphicsObject obj : gObjects) {
    ...
    public Rectangle getBounds(){
        return new Rectangle((int) Math.ceil(this.x + bounds.getX()), (int) Math.ceil(this.y + bounds.getY()), (int) Math.ceil(bounds.getWidth()), (int) Math.ceil(bounds.getHeight()));
    }
    ...
    private void updateBounds(){
        ...
        for(GraphicsObject gObject: gObjects) {
            if (newBounds != null) {
                ...
                Rectangle objBounds = gObject.getBounds();
                if (objBounds != null) {
                    newBounds = new Rectangle((int) objBounds.getX(), (int) objBounds.getY(), (int) objBounds.getWidth(), (int) objBounds.getHeight());
                }
            }
        }
        bounds = newBounds;
